...
SOURCE_LOCATIONS = [
    ...
    "Haymarket Square", "North End", "North Station",
    "South Station", "Theatre District", "West End"
]

DESTINATION_LOCATIONS = [
    "Beacon Hill","Back Bay", "Fenway", "Haymarket Square",
...
def load_distance_map():
    df = pd.read_csv("final_source_destination_distances.csv")
    df["source"] = df["source"].str.strip().str.lower()
    df["destination"] = df["destination"].str.strip().str.lower()
    return df

distance_map_df = load_distance_map()

def get_distance(source, destination):
    match = distance_map_df[
        (distance_map_df["source"] == source.lower()) &
        (distance_map_df["destination"] == destination.lower())
    ]
    return None if match.empty else float(match.iloc[0]["max_distance"])


# =============================================================================
# SURGE MULTIPLIER AUTOMATION - NEW ADDITION
# =============================================================================
def calculate_surge_multiplier(hour, day_of_week):
    """
    Calculate surge multiplier based on time conditions
    ...
    Returns:
        float: Surge multiplier (1.0 - 3.0)
    """
    base_surge = 1.0
    
    # Weekend check (Friday night, Saturday, Sunday)
    is_weekend = day_of_week in [1, 6, 7]  # Sun=1, Fri=6, Sat=7
    ...
    if 7 <= hour <= 9:  # Morning rush
        ...
        base_surge = 1.5
    elif 10 <= hour <= 16:  # Mid-day (off-peak)
        base_surge = 1.0
    else:  # Other hours
        base_surge = 1.25
    ...
    # Weekend multiplier
    if is_weekend:
        if 22 <= hour or hour <= 3:  # Weekend nights
            base_surge = min(base_surge * 1.4, 3.0)
        elif 10 <= hour <= 14:  # Weekend brunch time
            base_surge = min(base_surge * 1.2, 2.5)
    
    # Round to nearest 0.25
    base_surge = round(base_surge * 4) / 4
    
    # Ensure within bounds
    return max(1.0, min(3.0, base_surge))
...
st.markdown('<div class="main-header">🚕 Cab Ride Price Prediction System</div>', unsafe_allow_html=True)
...
with tab1:
    ...
    if pipeline_model is None or lr_model is None:
        ...
        with col1:
            ...
            source = st.selectbox("Source Location", SOURCE_LOCATIONS)
            destination = st.selectbox("Destination Location", DESTINATION_LOCATIONS)
            if source == destination:
                st.error("Source and Destination should not be same.")
            else: 
                distance = get_distance(source, destination)

                if distance is None:
                    st.error("Distance not found for selected route")
                    # st.stop()
        ...
        with col2:
            ...
            # NEW: Auto-calculate surge based on selected hour and day_of_week
            auto_surge = calculate_surge_multiplier(hour, day_of_week)
            
            surge_multiplier = st.slider(
                "Surge Multiplier",
                1.0, 3.0, auto_surge, 0.25,
                help=f"Higher during peak hours or high demand. Auto-calculated: {auto_surge}x based on selected time"
            )
...
with tab3:
    ...
    with col1:
        ...
        scenario_distance = st.slider(
            "Distance (miles)", 1, 30, 10, key="scenario_dist"
        )
        base_rate = st.number_input(
            "Base Rate ($/mile)", 1.0, 5.0, 2.5, step=0.1, key="base_rate"
        )
        base_fee = st.number_input(
            "Base Fee ($)", 0.0, 10.0, 5.0, step=0.5, key="base_fee"
        )
...
<div style='text-align: center; color: #666; padding: 2rem;'>
    <p style='font-size: 1.2rem; font-weight: 600;'>🚕 Cab Price Prediction System | Built with PySpark & Streamlit</p>
